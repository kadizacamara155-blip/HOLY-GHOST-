...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '224612635303@s.whatsapp.net'; // ton numÃ©ro ou le groupe cible
...
â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—
      *DigiX Bot Connected Successfully* ðŸš€
â• â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•£
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 224612635303; // mettez votre numÃ©ro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '224612635303' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'GHOSTMAN');
                console.log('ðŸ“² Pairing Code:', code);
                console.log('ðŸ‘‰ Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['224612635303@s.whatsapp.net'], // emplace par ton numÃ©ro WhatsApp 
